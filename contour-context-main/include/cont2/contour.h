...
struct ContourView
{
                       ...
                       const ContourSimThresConfig &simthres)
  {
    ...
    if (diff_perc<float>(cont_src.cell_cnt_, cont_tgt.cell_cnt_, simthres.tp_cell_cnt) && diff_delt<float>(cont_src.cell_cnt_, cont_tgt.cell_cnt_, simthres.ta_cell_cnt))
    {
#if HUMAN_READABLE
  ...
  addContourRes(const ContourView &cont1, const ContourView &cont2, const ContourViewStatConfig &cfg)
  {
    ...
    media.cell_vol3_torq_ = (cont1.com_ * (cont1.cell_cnt_ * cont1.vol3_mean_) + cont2.com_ * (cont2.cell_cnt_ * cont2.vol3_mean_)).cast<double>();
    media.cell_pos_tss_ =
        (cont1.pos_cov_ * (cont1.cell_cnt_ - 1) + cont1.cell_cnt_ * cont1.pos_mean_ * cont1.pos_mean_.transpose() + cont2.pos_cov_ * (cont2.cell_cnt_ - 1) + cont2.cell_cnt_ * cont2.pos_mean_ * cont2.pos_mean_.transpose()).cast<double>();
