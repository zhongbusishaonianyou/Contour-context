...
struct GMMPair
{
  ...
  // 这个代价函数实际上是在计算两个椭圆集合之间的负对数似然（negative log-likelihood），其中每个椭圆代表一个高斯分布。
  //它评估的是在给定变换参数下，源椭圆集合与目标椭圆集合匹配的程度。
  template <typename T>
...
public:
  ...
  double initProblem(const ContourManager &cm_src, const ContourManager &cm_tgt, const Eigen::Isometry2d &T_init)
  {
    ...
    //GMMPair：用户自定义的 代价函数类（必须实现 operator()）
