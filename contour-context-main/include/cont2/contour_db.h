...
struct CandidateManager
{
                     ...
                     const std::vector<float> &sim_area_perc)
    {
      ...
      for (int i = 0; i < anch_props_.size(); i++)
      {
        ...
        if (delta_T.translation().norm() < 2.0 && std::abs(std::atan2(delta_T(1, 0), delta_T(0, 0))) < 0.3)
        {
          ...
          Eigen::Vector2d trans_bl = (anch_props_[i].T_delta_.translation() * w1 + T_prop.translation() * w2) / (w1 + w2);
  ...
  CandidateManager(std::shared_ptr<const ContourManager> cm_q, const CandidateScoreEnsemble sim_lb, 
                   const CandidateScoreEnsemble sim_ub) : cm_tgt_(std::move(cm_q)), sim_var_(sim_lb), sim_ub_(sim_ub)
  {
  ...
  /**
   * * @brief checkCandWithHint
   * * @param cm_cand: the candidate contour manager
   * * @param anchor_pair: the constellation pair that is used to check the candidate
   * * @param cont_sim: the constellation similarity threshold
   */
                                           ...
                                           const ContourSimThresConfig &cont_sim)
  {
    ...
    Eigen::Isometry2d T_pass = ContourManager::getTFFromConstell(*cm_cand, *cm_tgt_, tmp_pairs2.begin(),tmp_pairs2.end());
  ...
  /// Main func 2/3:
  /**
   * * @brief tidyUpCandidates
   * * @details This function is called after all the candidates are added, and it will tidy up the candidates by
   * *  checking the correlation scores of each candidate.
   * * @param sim_var_ The dynamic lower bound of the similarity score.
   * * @param sim_ub_ The upper bound of the similarity score.
   */
  void tidyUpCandidates()
  {
    ...
    for (auto &candidate : candidates_)
    {
      ...
      auto corr_score_init = (float)corr_est->initProblem(*(candidate.cm_cand_), *cm_tgt_,candidate.anch_props_[0].T_delta_);
  ...
  /**
   * * @brief fineOptimize
   * * @details This function is called after all the candidates are tidied up, and it will fine-tune the candidates by
   * *  optimizing the correlation scores of each candidate.
   * * @param max_fine_opt The maximum number of candidates to be optimized.
   * * @param res_cand The resulting candidates after optimization.
   * * @param res_corr The resulting correlation scores after optimization.
   * * @param res_T The resulting transformation matrices after optimization.
   */ 
                   ...
                   std::vector<double> &res_corr, std::vector<Eigen::Isometry2d> &res_T)
  {
    ...
    // 第二次排序：根据优化后的相关性分数重新排序（仅排序前pre_sel_size个）
 ...
 * @description: 
 * @param q_ptr：查询点云
 * @param thres_lb :配置参数（下限）
 * @param thres_ub：配置参数（上限）
 * @param cand_ptrs：查询到的候选点云集
 * @param cand_corr：候选点云集的相关性
 * @param cand_tf：候选点云集的变换矩阵
 */
  void queryRangedKNN(const std::shared_ptr<const ContourManager> &q_ptr,
                      const CandidateScoreEnsemble &thres_lb,const CandidateScoreEnsemble &thres_ub,
                      std::vector<std::shared_ptr<const ContourManager>> &cand_ptrs,
                      std::vector<double> &cand_corr,std::vector<Eigen::Isometry2d> &cand_tf) const
  {
    cand_ptrs.clear();
    cand_corr.clear();
    cand_tf.clear();
    ...
    // for each layer（q_levels=·1,2,3「）
    for (int ll = 0; ll < cfg_.q_levels_.size(); ll++)
    { 
      //BCI对应论文中的公式（5），二进制距离向量
      ...
      { 
        ...
        if (q_keys[seq].sum() != 0)
        {
          ...
          {
            clk.tic();
            //addScan函数中已经将所有的候选点云BEV都添加到数据库all_bevs_中
            //cont_sim_cfg_中参数对应论文中的公式（4）
            //输入参数：候选帧索引，contour中anchor所处的层，该层的序号
            cand_mng.checkCandWithHint(all_bevs_[sear_res.first.gidx],ConstellationPair(cfg_.q_levels_[ll], sear_res.first.seq,seq),
                                       cfg_.cont_sim_cfg_);
            t2 += clk.toc();
          }
          stp.record("Constell");
