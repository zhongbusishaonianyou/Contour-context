...
public:
  ...
  int spinOnce()
  {
    ...
    {
      // 选择相关度最高的候选帧作为匹配帧，bev_tfs为当前帧到匹配帧的变换矩阵
...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ros::spinOnce();
    // spinOnce 回环检测的主函数
    int ret_code = o.spinOnce();
    if (ret_code == -2)
      ros::Duration(1.0).sleep();
    else if (ret_code == -1 || ret_code == 1)
      break;
